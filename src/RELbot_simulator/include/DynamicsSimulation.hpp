...
/**
 * @brief First-order dynamic system for the orientation of the camera,
 * including end-stops
 */
...
public:
   ...
   *
   * @param parent_node the parent node (RELbot sim) which contains the actual
   * ROS-connections
  ...
  /**
   * @brief Construct a new Dynamics Simulation object. Default constructor, not
   * implemented
   *
   */
...
private:
  ...
  const double DEFAULT_TIME_CONSTANT =
      0.3; // [s] Time constant to make motor velocities behave as 1st order
           // system
  // const bool DEFAULT_USE_TWIST_CMD = false;  // Which command mode (true is
  // Twist, false is Individual Motors)
  ...
  double thetaWorld_ =
      0.0; // [rad] Base link angle (about the z-axis) w.r.t. the world frame
  double angleRightMotor_ =
      0.0; // [rad] Current angle of the right motor w.r.t. base frame
  double angleLeftMotor_ =
      0.0; // [rad] current angle of the left motor w.r.t. base frame
  double velRightMotor_ = 0.0; // [rad/s] Current angular velocity of the right
                               // motor w.r.t. base frame
  double velLeftMotor_ = 0.0;  // [rad/s] current angular velocity of the left
                               // motor w.r.t. base frame

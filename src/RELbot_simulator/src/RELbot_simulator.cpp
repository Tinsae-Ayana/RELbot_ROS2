...
void RELbotSimulator::create_topics() {
  declare_parameter<bool>("use_twist_cmd",
                          RELbotSimulator::DEFAULT_USE_TWIST_CMD);

  RCLCPP_INFO(this->get_logger(), "Creating topics...");
  RCLCPP_INFO(this->get_logger(), "Creating Publishers");

  // outputs
  moving_camera_output_topic_ = this->create_publisher<sensor_msgs::msg::Image>(
      "output/moving_camera", 1);
  camera_position_topic_ =
      this->create_publisher<geometry_msgs::msg::PointStamped>(
          "output/camera_position", 1);
  robot_pose_topic = this->create_publisher<geometry_msgs::msg::PoseStamped>(
      "output/robot_pose", 1);
  ...
  RCLCPP_INFO(this->get_logger(), "Subscribing to %s",
              RELbotSimulator::WEBCAM_IMAGE.c_str());
  ...
  if (RELbotSimulator::useTwistCmd_) {
    ...
    const std::string right_motor_setpoint_vel_topic =
        RELbotSimulator::RIGHT_MOTOR_NAMESPACE +
        RELbotSimulator::SETPOINT_VEL_TOPIC;
    RELbotSimulator::rightMotorSetpointVelSubscriber_ =
        create_subscription<std_msgs::msg::Float64>(
            right_motor_setpoint_vel_topic, 10,
            std::bind(&RELbotSimulator::rightMotorSetpointVelCallback, this,
                      std::placeholders::_1));
    RCLCPP_INFO(get_logger(), "Subscribed to topic %s",
                right_motor_setpoint_vel_topic.c_str());

    const std::string left_motor_setpoint_vel_topic =
        RELbotSimulator::LEFT_MOTOR_NAMESPACE +
        RELbotSimulator::SETPOINT_VEL_TOPIC;
    RELbotSimulator::leftMotorSetpointVelSubscriber_ =
        create_subscription<std_msgs::msg::Float64>(
            left_motor_setpoint_vel_topic, 10,
            std::bind(&RELbotSimulator::leftMotorSetpointVelCallback, this,
                      std::placeholders::_1));
    RCLCPP_INFO(get_logger(), "Subscribed to topic %s",
                left_motor_setpoint_vel_topic.c_str());
  }
}

void RELbotSimulator::webcam_topic_callback(
    const sensor_msgs::msg::Image::SharedPtr msg_cam_img) {
  // RCLCPP_INFO(this->get_logger(), "Received webcam frame");

  /*
  Assumption:
  At init, Right-Hand (RH) reference frame or RELbot and world are equal. Relbot
  moving will shift X (forward/backward), Y (left/right). Theta describes angle
  w.r.t. world. GOAL: Forward     - Image shrinks Backwards   - Image 'grows',
  can over-extend to be img + black padding Left/Right  - Nothing (assume img is
  ...
  */
  ...
  double theta =
      dynamics_simulation_.get_theta(); // Positive = counter-clockwise rotation

  const double height = msg_cam_img->height;
  const double width = msg_cam_img->width;

  const double img_width_rad =
      120 * (M_PI / 180); // Define 'radial width'  of full cam img)
  const double pixels_per_rad =
      (width / 2) / img_width_rad; // convert to cam img we get

  int center_pixel_x =
      (int)(msg_cam_img->width / 2) -
      (theta * pixels_per_rad); // transform into what our center_x should be
  ...
  // Set header timestamp, useful for ordering messages if they are somehow
  // jumbled
...
cv::Mat RELbotSimulator::CreateCVSubimage(
  ...
  cv::Mat cv_frame =
      cv_bridge::toCvCopy(msg_cam_img, "bgr8" /* or other encoding */)->image;

  cv::Size size = cv_frame.size();

  const int window_dim =
      (int)(msg_cam_img->height /
            2); // Dirty calculation of window size we want to show in
  ...
  leftmost_pixel_x =
      std::clamp(leftmost_pixel_x, 0, size.width - output_image_dim);
  topmost_pixel_y =
      std::clamp(topmost_pixel_y, 0, size.height - output_image_dim);

  // Define the rectangle of interest (pixel location + output width (aka,
  // zoom))
  cv::Rect rect = cv::Rect(leftmost_pixel_x, topmost_pixel_y, output_image_dim,
                           output_image_dim);
  // turn that into valid sub-image
  cv::Mat subImg = cv_frame(rect);
  // resize into default size (so the window stays the same size)
  cv::resize(subImg, resized_frame, cv::Size(window_dim, window_dim),
             cv::INTER_LINEAR);
...
void RELbotSimulator::cmdVelCallback(
    const geometry_msgs::msg::TwistStamped::SharedPtr cmdVel) {
  ...
  dynamics_simulation_.set_vel_right_motor_set_point(
      (linearVelSetpoint + angularVelSetpoint * wheelBaseWidth_ / 2) /
      wheelRadius_);
  dynamics_simulation_.set_vel_left_motor_set_point(
      (linearVelSetpoint - angularVelSetpoint * wheelBaseWidth_ / 2) /
      wheelRadius_);
}
...
void RELbotSimulator::dynamics_timer_callback() {
  ...
  // RCLCPP_INFO(this->get_logger(), "Dynamics timer!   [x,y] = [%f,%f]", pos.x,
  // pos.y);
}

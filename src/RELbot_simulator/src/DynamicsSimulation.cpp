...
DynamicsSimulation::DynamicsSimulation(rclcpp::Node *parent_node,
                                       double time_step)
...
void DynamicsSimulation::step() {
  ...
  double angularVel =
      wheelRadius_ * (velRightMotor_ - velLeftMotor_) / wheelBaseWidth_;
  ...
  double velRightMotorDot =
      (velRightMotorSetpoint_ - velRightMotor_) / timeConstant_;
  double velLeftMotorDot =
      (velLeftMotorSetpoint_ - velLeftMotor_) / timeConstant_;
...
double DynamicsSimulation::get_x() { return xWorld_; }

double DynamicsSimulation::get_y() { return yWorld_; }

double DynamicsSimulation::get_theta() { return thetaWorld_; }
